...
class BaseNMFSinusoidsContinuum(object):
    ...
    def __init__(
        ...
        self.deg = deg
        if L is None:
            L = 2 * np.max(list(map(np.ptp, self.regions)))
        self.L = L        
    ...
    def fit(self, flux: np.ndarray, ivar: Optional[np.ndarray] = None, x0=None, full_output=False, **kwargs):
...
class HERMESContinuumModel(BaseNMFSinusoidsContinuum):
    ...
    def __init__(
        ...
        dispersion = 10 * (10**(2.57671464 + np.arange(167283) * 2.25855074e-06))
        components = load_components(components_path, dispersion.size - 2 * pad, pad=pad)        
        super(HERMESContinuumModel, self).__init__(
            dispersion,
            components,
            deg=deg,
            L=L,
