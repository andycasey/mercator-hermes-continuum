...
paths = [
    ...
    ("data/01067079_HRF_OBJ_ext_CosmicsRemoved_log_merged_c.fits", 70),
    ("data/01015922_HRF_OBJ_ext_CosmicsRemoved_log_merged_c.fits", 107.5),
    ("data/00984687_HRF_OBJ_ext_CosmicsRemoved_log_merged_c.fits", 200)
...
for path, v_rad in paths:
    ...
    model = HERMESContinuumModel(
        ...
        regions=[
            #[4000, 5000],
            #[6500, 6600]
            [3850, 5000] # Paschen jump not well modelled by the BOSZ grid.
        ]
    )
    ...
    interp_flux = np.interp(model.dispersion, wavelength, flux, left=np.nan, right=np.nan)
    ...
    is_likely_telluric = (
        ((5850 < model.dispersion) & (model.dispersion < 6100))
    |   ((6250 < model.dispersion) & (model.dispersion < 6350))
    |   ((6860 < model.dispersion) & (model.dispersion < 7440))
    |   ((7590 < model.dispersion) & (model.dispersion < 7750))
    )
    ...
    ax.plot(model.dispersion, interp_flux, c='k')
    ax.plot(model.dispersion, continuum[0], c="tab:blue")

    ax_norm.plot(model.dispersion, interp_flux / continuum[0], c="k")
    xlim = ax_norm.get_xlim()
    ax_norm.plot(model.dispersion, meta["rectified_model_flux"], c="tab:red")
    ax_norm.set_xlim(xlim) # restrict to just the region we fit
    ax_norm.set_ylim(0, 1.2)
    ax_norm.set_xlabel("wavelength [vac; A]")
    
    fig.savefig(path.replace(".fits", "_5000.png"))
